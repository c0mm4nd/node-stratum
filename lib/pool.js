...
module.exports = class Pool extends events.EventEmitter {
    ...
    GetFirstJob(finishedCallback) {
        this.GetBlockTemplate((error) => {
            if (error) {
                console.error("Error with getblocktemplate on creating first job, server cannot start");
                return;
            }
    ...
    OnBlockchainSynced(syncedCallback) {
        ...
        let checkSynced = (displayNotSynced) => {
            this.daemonManager.cmd("getblocktemplate", [{
                "capabilities": ["coinbasetxn", "workid", "coinbase/append"],
                "rules": ["segwit"]
            }], (results) => {
                let synced = results.every((r) => {
    ...
    SubmitBlock(blockHex, callback) {
        ...
        this.daemonManager.cmd(rpcCommand,
            rpcArgs,
            (results) => {
                for (let i = 0; i < results.length; i++){
                    const result = results[i];
                    if (result.error) {
                        console.error("rpc error with daemon instance " +
                            result.instance.index + " when submitting block with " + rpcCommand + " " +
                            JSON.stringify(result.error)
                        );
                        return;
                    } else if (result.response === "rejected") {
                        console.error("Daemon instance " + result.instance.index + " rejected a supposedly valid block");
                        return;
                    }
    ...
    SetupDaemonManager(finishedCallback) {
        ...
        this.daemonManager = new DaemonManager(this.options.daemons, (severity, message) => {
            this.emit("log", severity, message);
        });

        this.daemonManager.once("online", () => {
            finishedCallback();
    ...
    DetectCoinData(finishedCallback) {
        ...
        this.daemonManager.batchCmd(batchRpcCalls, (error, results) => {
            if (error || !results){
                console.error("Could not start pool, error with init batch RPC call: " + JSON.stringify(error));
                return;
            }
            ...
            for (let i = 0; i < results.length; i++){
                ...
                if (rpcCall !== "submitblock" && (r.error || !r.result)) {
                    console.error("Could not start pool, error with init RPC " + rpcCall + " - " + JSON.stringify(r.error));
                    return;
                }
            }

            if (!rpcResults.validateaddress.isvalid){
                console.error("Daemon reports address is not valid");
                return;
            }
            ...
            if (rpcResults.submitblock.message === "Method not found") {
                ...
                console.error("Could not detect block submission RPC method, " + JSON.stringify(results));
                return;
            }
    ...
    StartStratumServer(finishedCallback) {
        ...
        this.stratumServer.on("started", () => {
            ...
            client.on("difficultyChanged", (diff) => {
                ...
                console.warn("Booted worker " + client.getLabel() + " who was connected from an IP address that was just banned");
    ...
    CheckBlockAccepted(blockHash, callback) {
        this.daemonManager.cmd("getblock", [blockHash],
            (results) => {
